...
| **Adapter (transport)**          | `mock` / `jwt` / `oauth` factories inside `useAuth.ts`                                                                                                     | Translates auth operations into HTTP calls. Pluggable per environment.                      |
...
stateDiagram-v2
  ...
  Anonymous --> LockedOut: 5 failures / 15 min
  LockedOut --> Anonymous: 15 min cool-off
```
...
When `VUESTRATA_AUTH_ADAPTER=mock`, the mock adapter supplements the in-memory store with **IndexedDB persistence** so demo sessions survive page reloads:
...
- `src/modules/app/state/demo-storage.ts` — wraps records in a tamper-evident `DemoEnvelope` (SHA-256 HMAC, version, `expiresAt`). Any envelope whose hash fails validation is discarded fail-closed.
- `src/modules/app/state/demo-store.ts` — typed `getDemoSession` / `setDemoSession` / `clearDemoSession` used by the mock adapter and `restoreSession` in `main.ts`.
- Cross-tab sync via `BroadcastChannel('vuestrata-demo-auth')`.

This path is entirely separate from the production token lifecycle. Real adapters (jwt/oauth) never touch IndexedDB.
...
The `AuthAdapter` interface is the only contract every transport must
satisfy:
...
Hits the MSW handlers in [src/modules/app/mocks/](../../src/modules/app/mocks/).
Useful for development and tests; persists nothing on the server.

### JWT adapter

Wraps the mock adapter and adds **token-expiry awareness** before each call:

```ts
function isTokenExpired(token: string): boolean {
  const payload = jwtDecode<{ exp?: number }>(token)
  if (!payload.exp) return false
  return Date.now() >= (payload.exp - 30) * 1000 // 30 s safety window
}
```

`getUser` short-circuits (returns `null`) when the token is expired, which
makes session restoration safe even if the API would otherwise 401.
...
stateDiagram-v2
  ...
  Counting --> Counting: failure (n<5, within 15 min)
  Counting --> Idle: success / window expires
  Counting --> Locked: 5th failure
  Locked --> Idle: 15 min elapsed
  Locked --> Locked: login attempt -> throw LockoutError
```

### Lockout

After **5 failed attempts within 15 min**, login is blocked for 15 min.
...
| **Brute-force login**                     | Per-session lockout (5 attempts / 15 min → 15 min cool-off), persisted in `sessionStorage`.                                                                                                                                           |
...
- **A02 Cryptographic Failures** — PKCE for OAuth; tokens never persisted.
...
├── composables/
│   ├── useAuth.ts                  # adapter selection, login/register/MFA/OAuth/lockout
│   └── useRbac.ts                  # component-facing permission API
└── plugins/
    ├── router.ts                   # auth + RBAC navigation guards
    └── bootstrap-theme.ts          # pre-mount FOUC guard (theme, dark, locale)

src/modules/core/lib/
├── api/
│   ├── client.ts                   # apiFetch + CSRF + 5xx hardening
│   ├── auth-interceptor.ts         # 401 → refresh dedup + cooldown
│   └── types.ts                    # ApiAuthProvider contract
├── auth/pkce.ts                    # OAuth PKCE helpers
├── rbac/
...
├── errors/index.ts                 # AppError + normalizeError (5xx hardening)
