...
The router must be installed before `setupModules(...)` because module setup calls
`router.addRoute(...)`. Pinia must be installed before the router guard or module store reads any
stores.
...
```text
src/modules/app/pages/about.vue -> /about
src/modules/app/pages/docs/[...slug].vue -> /docs/:slug(.*)
```
...
- Do not call `setupModules(...)` before `app.use(router)` and `app.use(pinia)`.
...
| `src/main.ts`                       | Installs router and calls `setupModules(...)` before mount.                    |
