...
Vue Router's Vite plugin scans **only** these two roots. Domain-module pages
(`src/modules/customers/pages/index.vue` and friends) are deliberately _not_ file-routed — they
reach the router through their module's `routes` array instead, which is what attaches the layout
wrapper and the RBAC meta.

> A third entry, `{ src: 'src/modules/*/pages' }`, used to sit in this list. The plugin does not
> expand that glob, so it contributed zero routes — `typed-router.d.ts` never contained a single
> module page. It has been removed; if you are looking for a module page in the generated types,
> that is why it was never there.
...
The `docs` layout is English/LTR only. It hides the header language switcher and
`resolveActiveLocale()` in `src/modules/app/plugins/appearance.ts` keeps
`html[lang]`/`html[dir]` on English for every `/docs` URL. The persisted locale
is not written, so leaving the docs restores French or Arabic.
