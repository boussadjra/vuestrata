...
import { useMoveTaskMutation, useProjectQuery, useTasksQuery } from '../composables/useProjects'
...
const { item: project, isPending: projectPending } = useProjectQuery(projectId)

useBreadcrumbLabel(() => project.value?.name)

const filters = computed(() => ({
  projectId: projectId.value,
  // One page big enough for a whole board. A paginated kanban is not a kanban:
  // "page 2 of In Progress" is not a concept the user has.
  pageSize: 100,
  sortBy: 'position',
  sortOrder: 'asc' as const,
}))

const { items: tasks, isPending, isError, refetch } = useTasksQuery(filters)
const moveTask = useMoveTaskMutation()

/** The board is derived from `TASK_STATUSES`, so a new status cannot hide tasks. */
const columns = computed(() =>
  TASK_STATUSES.map((status) => ({
    status,
    label: t(`projects_status_${status}`),
    tasks: tasks.value
...
const PRIORITY_VARIANT: Record<TaskPriority, 'default' | 'secondary' | 'warning' | 'error'> = {
  low: 'default',
  medium: 'secondary',
  high: 'warning',
  urgent: 'error',
...
function destinationsFor(task: Task) {
  return TASK_STATUSES.filter((status) => status !== task.status).map((status) => ({
...
/**
 * `UiSelect` emits the widest value type it supports (string | number | array).
 * The options here are only ever task statuses, so the value is narrowed with a
 * membership check rather than a cast — a value that is not a status is a bug
 * worth ignoring, not one worth crashing the board over.
 */
function move(task: Task, value: string | number | (string | number)[]) {
  const candidate = Array.isArray(value) ? value[0] : value
  if (typeof candidate !== 'string') return
  if (!(TASK_STATUSES as readonly string[]).includes(candidate)) return

  const target = candidate as TaskStatus
  const column = columns.value.find((entry) => entry.status === target)
  // Appended to the end of the destination column, one step past the last
  // card. Reusing an existing position makes the order between the two
  // undefined, and they swap on reload.
  const last = column?.tasks.at(-1)?.position ?? 0
  moveTask.mutate({ id: task.id, patch: { status: target, position: last + 1000 } })
}

/**
 * Overdue is computed against today, not stored.
 * A stored flag is correct only until midnight.
 */
function isOverdue(task: Task): boolean {
  return task.dueAt !== null && task.status !== 'done' && Date.parse(task.dueAt) < Date.now()
}
</script>
...
<template>
  <div class="space-y-6">
    ...
    <div
      v-else-if="isPending || projectPending"
          ...
          <li v-for="task in column.tasks" :key="task.id">
            <UiCard class="space-y-2 p-3">
              ...
              <div class="flex flex-wrap items-center gap-1.5">
                <UiBadge :variant="PRIORITY_VARIANT[task.priority]" size="sm">
                ...
                -->
                <UiBadge v-if="isOverdue(task)" variant="error" size="sm">
                  {{ t('projects_overdue') }}
                </UiBadge>
              </div>
              ...
              <UiSelect
                ...
                size="sm"
                @update:model-value="(value) => move(task, value)"
              />
            </UiCard>
          </li>
        </ul>
